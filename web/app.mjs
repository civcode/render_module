...
function start() {
    ...
    function layout() {
        ...
        if (!rect) return;
        for (const element of [canvas, surface]) Object.assign(element.style,
    ...
    function connect() {
        ...
        current.onopen = () => { if (socket === current) { attempt = 0; connection.textContent = 'Connected'; } };
        current.onmessage = event => {
            if (socket !== current) return;
            try {
                if (typeof event.data === 'string') {
                    const message = JSON.parse(event.data); if (message.v !== 1) throw Error('version');
                    if (message.type === 'welcome') { connection.dataset.session = message.session; setController(message.control); }
        ...
        current.onclose = () => {
            if (socket !== current) return;
            release(false); setController(false); pending = null; connection.textContent = 'Disconnected · retrying';
