...
public:
    ...
    using ImageViewCallback = render_module::ImageViewCallback;
    ...
    // The provider is sampled on the render thread whenever the window is
    // visible. The LatestImage overload is the normal camera-thread handoff.
    static void RegisterImageView(
        const std::string& name,
        render_module::ImageProvider provider,
        ImageViewCallback overlayCallback = {},
        const render_module::ImageViewOptions& options = {});
    static void RegisterImageView(
        const std::string& name,
        std::shared_ptr<render_module::LatestImage> source,
        ImageViewCallback overlayCallback = {},
        const render_module::ImageViewOptions& options = {});
